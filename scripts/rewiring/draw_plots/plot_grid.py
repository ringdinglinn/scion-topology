...
from scripts.rewiring.draw_plots.utils import apply_styling, METRIC_NAMES, METRIC_LABELS
...
def plot_metrics(metrics, groups, output_dir, title=None):
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(4 * n_cols, 3 * n_rows))  # smaller
    ...
    for i, group_name in enumerate(group_names):
        ...
        for sub_name, rows in subgroups.items():
            for m_idx, metric in enumerate(metrics):
                ...
                line, = ax.plot(
                    ...
                    marker="o", markersize=2, color=color_map[sub_name],
        ...
        ax.set_ylabel(" | ".join(metric_labels) if col == 0 else "")
    ...
    if handles:
        fig.legend(handles, labels, loc="lower center", ncol=min(len(labels), 5),
                   bbox_to_anchor=(0.5, -0.01))  # closer to plots
