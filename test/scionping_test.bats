...
@test "scion2-3 can scion-ping scion1-3" {
	current_node="scion2-3"
	run docker exec monitor scionctl scionping start scion2-3 scion1-3 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
...
@test "scion2-4 can scion-ping scion1-4" {
	current_node="scion2-4"
	run docker exec monitor scionctl scionping start scion2-4 scion1-4 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
...
@test "scion2-5 can scion-ping scion1-5" {
	current_node="scion2-5"
	run docker exec monitor scionctl scionping start scion2-5 scion1-5 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
...
@test "scion2-6 can scion-ping scion1-6" {
	current_node="scion2-6"
	run docker exec monitor scionctl scionping start scion2-6 scion1-6 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
...
@test "scion2-7 can scion-ping scion1-7" {
	current_node="scion2-7"
	run docker exec monitor scionctl scionping start scion2-7 scion1-7 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
...
@test "scion2-8 can scion-ping scion1-8" {
	current_node="scion2-8"
	run docker exec monitor scionctl scionping start scion2-8 scion1-8 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
	run docker exec monitor scionctl scionping list scion2-8
	filename=$(echo "$output" | awk -F'|' '/\.log/ {gsub(/ /, "", $3); fname=$3} END {sub(/\.log$/, "", fname); print fname}')
	run docker exec monitor scionctl scionping file scion2-8 $filename
	ping_output="$output"
	packet_loss_line=$(echo "$ping_output" | grep "packet loss")
	packet_loss_value=$(echo "$packet_loss_line" | awk '{print $6}')
	[ "$packet_loss_value" = "0%" ]
}
@test "scion2-9 can scion-ping scion1-9" {
	current_node="scion2-9"
	run docker exec monitor scionctl scionping start scion2-9 scion1-9 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
	run docker exec monitor scionctl scionping list scion2-9
	filename=$(echo "$output" | awk -F'|' '/\.log/ {gsub(/ /, "", $3); fname=$3} END {sub(/\.log$/, "", fname); print fname}')
	run docker exec monitor scionctl scionping file scion2-9 $filename
	ping_output="$output"
	packet_loss_line=$(echo "$ping_output" | grep "packet loss")
	packet_loss_value=$(echo "$packet_loss_line" | awk '{print $6}')
	[ "$packet_loss_value" = "0%" ]
}
@test "scion2-10 can scion-ping scion1-10" {
	current_node="scion2-10"
	run docker exec monitor scionctl scionping start scion2-10 scion1-10 --count 1
	sleep 1
	[ "$status" -eq 0 ]
	[[ "$output" != *"Error"* ]]
	run docker exec monitor scionctl scionping list scion2-10
	filename=$(echo "$output" | awk -F'|' '/\.log/ {gsub(/ /, "", $3); fname=$3} END {sub(/\.log$/, "", fname); print fname}')
	run docker exec monitor scionctl scionping file scion2-10 $filename
	ping_output="$output"
	packet_loss_line=$(echo "$ping_output" | grep "packet loss")
	packet_loss_value=$(echo "$packet_loss_line" | awk '{print $6}')
	[ "$packet_loss_value" = "0%" ]
